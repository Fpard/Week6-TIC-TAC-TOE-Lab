...
public class Week06TicTacToeLab {
	...
	//	MY IMPLEMENTATION
	//  
	//	This is the driver for the game. It uses two other classes: GameBoard and Player.
	
	//  GameBoard initializes and displays the board, processes the actions in the game, 
	//  checks for the winner and stores all winning combinations.
	
	//  Player performs and stores its own moves, checks for player inputs for validity, and stores a player's moves every third turn. 
	...
	public static void main(String[] args) {
        ...
        while(play) {
			        ...
			        int playerX_Choice, playerO_Choice;
			        boolean winner = false;
			       
			        do{
			        	
			        	if(!winner){
			        			playerX_Choice = playerX.playerMoves(theBoard);
			        			theBoard.processGame(playerX, playerX_Choice);
			        			winner = theBoard.checkGameStatus(playerX);
			        			if (winner) {
			        				System.out.println(playerX.getName() + "  is the winner");
				        		
			        			} else if (GameBoard.turns < 9 && GameBoard.turns > 4) {
			        	...
			        	if (!winner) {
			        		playerO_Choice = playerO.playerMoves(theBoard);
			        		theBoard.processGame(playerO, playerO_Choice);
			        		winner = theBoard.checkGameStatus(playerO);
			        		if (winner) {
			        			System.out.println(playerO.getName() + "  is the winner");
			        		} else if (GameBoard.turns <= 9 &&  GameBoard.turns > 4) {
			        		     System.out.println(" No winner yet.");
			        		}
				       	}
			        	...
			        	if(GameBoard.turns > 9 && !winner) {
			        		System.out.println("It's a DRAW!");
			        		GameBoard.turns++;
			        	}
			        	
			        	
			        }while (!winner && GameBoard.turns < 10); //END DO WHILE
			        
			      System.out.println("Do you want to play another round? Yes (y) or No (n)");
			      
			      keepGoing = GameBoard.in.nextLine();
